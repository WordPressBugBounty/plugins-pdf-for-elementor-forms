...
?>
	...
	private function find_elementor_forms($elements, $post_id, &$found_forms)
	{
		foreach ($elements as $element) {
			if ('widget' === $element['elType'] && 'form' === $element['widgetType']) {
				$form_id = $element['id'] . '-' . $post_id;
				$form_name = !empty($element['settings']['form_name']) ? $element['settings']['form_name'] : 'Unnamed Form';
				$found_forms[] = [
					'id'    => $form_id,
					'title' => $form_name . ' - ' . $element['id']
				];
			}
			if (! empty($element['elements'])) {
				$this->find_elementor_forms($element['elements'], $post_id, $found_forms);
			}
		}
	}
	...
	{
		global $wpdb;
		$lists_pages = $this->get_list_forms();
		$current_form_id = get_post_meta($post->ID, '_pdfcreator_formwidget', true);
		$all_forms = [];
		foreach ($lists_pages as $page) {
			$document = \Elementor\Plugin::$instance->documents->get($page->ID);
			if (! $document) continue;
			$data = $document->get_elements_data();
			if (empty($data)) continue;
			$page_forms = [];
			$this->find_elementor_forms($data, $page->ID, $page_forms);
			foreach ($page_forms as $f) {
				$f['page_title'] = $page->post_title;
				$all_forms[] = $f;
			}
		}
		?>
					...
					if (count($form_parts) > 1) {
						...
						foreach ($results as $rs) {
							printf(
								...
								esc_attr($rs->id),
								selected($current_entry, $rs->id, false),
								esc_html($rs->created_at)
							);
						}
					}
				}
...
<?php
	...
	/**
	 * Adds Elementor Form fields as shortcodes for PDF templates.
	 * * @param array $shortcode Existing shortcodes array.
	 * @return array Updated shortcodes array.
	 */
	public function add_shortcode($shortcode)
	{
		...
		if ($post_id) {
			...
			$ids = explode("-", $form_data_meta);
			// Ensure we have both a Widget ID and a Page ID
			if (isset($ids[0], $ids[1]) && !empty($ids[0])) {
				$elementor_data = get_post_meta($ids[1], '_elementor_data', true);
				// Elementor data is stored as a JSON string
				$page_elements = is_string($elementor_data) ? json_decode($elementor_data, true) : $elementor_data;
				if ($page_elements) {
					// Use recursion to find the specific Form widget by its ID
					$form_widget = $this->find_widget_by_id_recursive($page_elements, $ids[0]);
					if ($form_widget && isset($form_widget['settings']['form_fields'])) {
						foreach ($form_widget['settings']['form_fields'] as $field) {
							// Use field label if available, otherwise fallback to the custom ID
							$label = !empty($field["field_label"]) ? $field["field_label"] : $field["custom_id"];
							$inner_shortcode["field id='" . $field["custom_id"] . "'"] = $label;
						}
					}
				}
			}
	...
	/**
	 * Recursively searches for an Elementor widget within the page data.
	 * * @param array  $elements  Array of Elementor sections/columns/widgets.
	 * @param string $target_id The ID of the widget we are looking for.
	 * @return array|null       The widget data if found, or null.
	 */
	private function find_widget_by_id_recursive($elements, $target_id)
	{
		foreach ($elements as $element) {
			// Check if this specific element is our target
			if (isset($element['id']) && $element['id'] === $target_id) {
				return $element;
			}
			// If the element contains child elements (sections/columns/widgets), dig deeper
			if (!empty($element['elements'])) {
				$found = $this->find_widget_by_id_recursive($element['elements'], $target_id);
				if ($found) {
					return $found;
				}
			}
